...
const Team = () => {
  ...
  const { user } = useSelector(state => state.authSlice); // Ensure you have this selector
  ...
  useEffect(() => {
    (async () => {
      ...
      console.log("Team Members Response:", res1); // Debugging line
      if (res1.success) {
        ...
        <section className="section">
          {
            ...
            <>
              ...
              <div className="row">
                ...
                <CountsCard title='Total Projects' icon='fa-project-diagram' count={team.information.projects} />
                <CountsCard title='Total Completed Tasks' icon='fa-check' count={team.information.completedTasks} />
                <CountsCard title='Total Pending Tasks' icon='fa-tasks' count={team.information.pendingTasks} />
              </div>
                    ...
                    <tbody>
                      {
                        teamMembers.length > 0 ?
                          teamMembers.map((item, index) => (
                            <RowMember key={index} index={index + 1} data={item} teamId={team.id} /> // Passing teamId
                          ))
                          :
                          <tr><td colSpan={user.type === "Admin" ? 7 : 6}>No Record Found</td></tr>
                      }
                    </tbody>
                  </table>
                </div>
